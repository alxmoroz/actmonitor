...
import 'package:amonitor/L3_app/extra/services.dart';
...
Future<DeviceModel?> selectModel(BuildContext context, DeviceModel? selectedModel, {bool comparisonMode = false}) async {
  DeviceModel? model;
  if (specsController.models.isNotEmpty) {
...
class _ModelsListState extends State<ModelsList> with TickerProviderStateMixin {
  ...
  Iterable<DeviceModel> get pageModels => specsController.knownModels.where((m) => m.type == selectedType);
            ...
            Expanded(
              child: SingleChildScrollView(
                ...
                child: SingleVariantSelection(
                  pageModels.map(
                    (model) {
                      final alreadySelected = widget.comparisonMode && comparisonController.comparisonModelsNames.contains(model.name);
